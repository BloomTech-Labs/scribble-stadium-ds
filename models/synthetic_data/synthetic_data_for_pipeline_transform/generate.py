...
def create_fake_photo_unmodified(pix_per_mm: float, lines: int, red: [] = [255, 164, 164], blue: [] = [128, 128, 255],
    ...
    #
    blue_lines_spacing = (paper_height_mm - (32. * 1.5)) / lines
    for bl in range(lines):
        noise_val = random.random() * (noise / 200)
        blue_lines_thickness_half = .25 + noise_val

        offset = (32 * 1.5) + (bl * blue_lines_spacing)
        blue_line_box = [0,
                         offset - blue_lines_thickness_half,
                         paper_width_mm,
                         offset + blue_lines_thickness_half
                         ]
        draw_box(blue_line_box, blue)

    ## draw ink, this simulates where writing is on the paper
    ink_lines_spacing = (paper_height_mm - (32. * 1.5)) / lines

    for il in range(lines):
        ink_lines_offset = (32 * 1.5) + (il * ink_lines_spacing) + (.5 * ink_lines_spacing)
        ...
        for x in range(int(32 * pix_per_mm), int(paper_width_mm * pix_per_mm)):
            ...
            ymax = int((ink_lines_spacing + (ink_lines_spacing / 2.5)) * pix_per_mm)
            for y in range(ymax):

                yy = (ink_lines_offset) * pix_per_mm
                ...
                if random.random() < probability_of_ink:
                    xx = x
                    tx = int(xx)
                    ty = int(yy + y - (ink_lines_spacing * pix_per_mm / 2))
                    if ty < img.shape[0]:
                        img[ty, tx] = ink
...
def randomize_photo_transform(img, photo_size, noise=1):
    ...
    return photo_img, pts2


def create_one(noise: float = 1, set_name: str = ""):
    """
