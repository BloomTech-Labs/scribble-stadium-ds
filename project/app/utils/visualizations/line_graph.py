...
def line_graph(score_history, name):
    ...
    fig = go.Figure(data=go.Scatter(x=[i+1 for i in range(len(score_history))],
                    y=score_history, line_width=7, line_color="#EB7E5B",
                    mode='lines+markers+text', marker_size=18, 
                    marker_color='#FED23E', marker_symbol='star'))
    ...
    fig.update_xaxes(title_text='Week Number',
    ...
    fig.update_yaxes(title_text='Squad Score',
                     title_font={'size': 20, 'family': 'PT Sans Narrow'},
